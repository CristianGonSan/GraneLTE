<form wire:submit.prevent="export">
    <div class="card">
        <div class="card-body">
            ...
            <div class="row">
                <x-form.select-wire-ignore fgroup-class="col-md-4" name="material_id_{{ $this->getId() }}" label="Material"
                    wire:loading.attr="readonly" wire:target="export" />

                <x-form.select-wire-ignore fgroup-class="col-md-4" name="category_id_{{ $this->getId() }}"
                    label="Categoría" wire:loading.attr="readonly" wire:target="export" />

                <x-form.select-wire-ignore fgroup-class="col-md-4" name="supplier_id_{{ $this->getId() }}"
                    label="Proveedor" wire:loading.attr="readonly" wire:target="export" />
            </div>

            {{-- Fila 2: Disponibilidad y fechas de recepción --}}
            <div class="row">
